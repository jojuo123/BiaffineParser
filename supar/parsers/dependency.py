...
from supar.models.threads import TwoThread
...
class BiaffineDependencyParser(Parser):
    ...
    def build(cls, path,
        ...
        FEAT2 = Field('tags', bos=bos)
        ARC = Field('arcs', bos=bos, use_vocab=False, fn=CoNLL.get_arcs)
        REL = Field('rels', bos=bos)
        if args.feat in ('char', 'bert'):
            transform = CoNLL(FORM=(WORD, FEAT), POS=FEAT2, HEAD=ARC, DEPREL=REL)
        else:
            transform = CoNLL(FORM=WORD, POS=FEAT, HEAD=ARC, DEPREL=REL)
        ...
        args.update({
            ...
            'n_feat_pos': len(FEAT2.vocab),
        })
        print('feat2', len(FEAT2.vocab))
        print('feat', len(FEAT.vocab))
...
class TwoThreadParser(Parser):
    ...
    def _train(self, loader):
        ...
        for words, feats, pos, arcs, rels in bar:
            ...
            # ignore the first token of each sentence
            mask[:, 0] = 0
            s_arc, s_rel = self.model(words, feats, pos)
            loss = self.model.loss(s_arc, s_rel, arcs, rels, mask, self.args.partial)
            loss.backward()
            nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip)
            self.optimizer.step()
    ...
    def _predict(self, loader):
        ...
        preds = {}
        arcs, rels, probs = [], [], []
        for words, feats, pos in progress_bar(loader):
            mask = words.ne(self.WORD.pad_index)
            # ignore the first token of each sentence
            mask[:, 0] = 0
            lens = mask.sum(1).tolist()
    ...
    def build(cls, path,
        ...
        if args.lexemetrain:
            if os.path.exists(args.pospath):
                state = torch.load(args.pospath)

        # model.load_pretrained(state['pretrained'])
        # model.load_state_dict(state['state_dict'], False)
        if args.lexemetrain:
            model = cls.MODEL(**args)
            model.load_pretrained(embed=WORD.embed, embed2=state['pretrained'])
            model.load_state_dict(state['state_dict'], False)
            model.freeze_unfreeze()
            model.to(args.device)
        elif args.postrain:
            model = cls.MODEL(**args).load_pretrained(embed=None, embed2=WORD.embed).to(args.device)


        #if (kwargs['lexemetrain']):
